...
export class ArquivoService {
  ...
  async dataExists(id: PocTypings.Arquivo["id"]) {
    const fileFullPath = this.datGetFilePath(id);
    return jetpack.exists(fileFullPath);
  }

  async dataReadAsStream(id: PocTypings.Arquivo["id"]): Promise<Readable | null> {
  ...
  async getFile(accessContext: AccessContext | null, id: PocTypings.Arquivo["id"], acesso: IGetFileAcesso | null) {
  ...
  async getStreamableFile(accessContext: AccessContext | null, id: PocTypings.Arquivo["id"], acesso: IGetFileAcesso | null) {
  ...
  async dataSave(id: PocTypings.Arquivo["id"], data: NodeJS.ArrayBufferView | Readable) {
    const fileFullPath = this.datGetFilePath(id);
    await writeFile(fileFullPath, data);
    return true;
  }

  async arquivoCreate(dto: Pick<PocTypings.Arquivo, "name" | "mimeType">, data: NodeJS.ArrayBufferView | Readable): Promise<Pick<ArquivoEntity, "id">> {
    let id: string;
  ...
  private datGetFilePath(id: PocTypings.Arquivo["id"]) {
